...
function tmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'bos-robotstxt-report-'));
}
...
function resultWith(severities = ['fail', 'warn']) {
  return new AuditResult(
    severities.map(
      (severity, index) =>
        new Finding({ ruleId: `RB00${index + 1}`, severity, message: `finding ${index}` }),
    ),
  );
}
...
describe('lib/findings', () => {
  it('derives a title, remediation, and help URI from the rule id', () => {
    const f = finding();
    assert.strictEqual(f.title, 'Sitemap directive present');
    assert.match(f.remediation, /Sitemap/);
  ...
  it('produces a stable finding key independent of message wording', () => {
    ...
    assert.match(finding().findingKey, /^rb002-[0-9a-f]{16}$/);
  });

  it('counts totals per severity', () => {
    const result = new AuditResult([
      finding({ severity: 'pass' }),
      finding({ ruleId: 'RB003', severity: 'warn' }),
      finding({ ruleId: 'RB010', severity: 'fail' }),
      finding({ ruleId: 'RB020', severity: 'error' }),
      finding({ ruleId: 'RB030', severity: 'skip' }),
    ]);
  ...
  it('buckets rule ids into display families', () => {
    assert.strictEqual(familyFor('RB001'), 0);
    assert.strictEqual(familyFor('RB010'), 1);
    assert.strictEqual(familyFor('RB020'), 2);
    assert.strictEqual(familyFor('RB030'), 3);
  ...
  it('renders a Markdown report with summary, recommendations, and details', () => {
    ...
    assert.match(md, /# Blackout Secure Robots TXT Generator Audit Report/);
    assert.match(md, /\*\*Verdict:\*\* Action required/);
    assert.match(md, /## Run Context/);
    assert.match(md, /### Directives/);
  });
...
describe('lib/sarif', () => {
  it('drops skip findings and keeps the rest', () => {
    const run = sarifMod.auditRun([
      ...
      finding({ ruleId: 'RB003', severity: 'skip' }),
      finding({ ruleId: 'RB010', severity: 'warn' }),
    ]);
  ...
  it('carries the finding key as a partial fingerprint', () => {
    const f = finding();
    assert.strictEqual(
      sarifMod.auditRun([f]).results[0].partialFingerprints.bosRobotsTxtFindingKey,
      f.findingKey,
    );
  });
  ...
  it('merges runs and round-trips through disk', () => {
    ...
    try {
      const merged = sarifMod.merge(
        ...
        { runs: [sarifMod.auditRun([finding({ ruleId: 'RB003' })])] },
      );
...
describe('lib/report', () => {
  ...
  it('prints grouped families and a totals line', () => {
    ...
    reportMod.printAuditTable(
      core,
      new AuditResult([
        finding({ ruleId: 'RB001', severity: 'pass' }),
        finding({ ruleId: 'RB010', severity: 'fail' }),
      ]),
    );
    const text = core.lines.map(([, m]) => m).join('\n');
    assert.match(text, /Directives/);
    assert.match(text, /Crawl policy/);
  ...
  it('writes the JSON report, recommendations, and skips sidecars', () => {
    const dir = tmpDir();
    try {
      const result = new AuditResult([finding(), finding({ ruleId: 'RB003', severity: 'skip' })], {
        group_count: 1,
      });
      ...
      assert.strictEqual(skips[0].rule_id, 'RB003');
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
...
describe('lib/ai', () => {
  ...
  it('prefers the dedicated models token and model overrides', () => {
    const provider = aiMod.detectProvider('github', {
      ...
      GITHUB_MODELS_MODEL_ROBOTS_TXT: 'openai/gpt-4.1-mini',
    });
  ...
  it('reports a clean run in the local summary', () => {
    assert.match(aiMod.localSummary(resultWith(['pass'])), /No robots.txt control requires/);
  });
...
describe('lib/metadata', () => {
  it('reports package identity from package.json', () => {
    ...
    assert.strictEqual(pkg.name, 'bos-robotstxt-generator');
...
describe('cli', () => {
  ...
  it('requires a site URL to resolve sitemap directives', () => {
    const cfg = cfgMod.resolve(os.tmpdir(), { useGlobalConfig: false });
    assert.throws(() => contentFromConfig(cfg, ''), /--site-url is required/);
  });
  ...
  it('prints usage with no command', async () => {
    ...
    assert.strictEqual(code, 0);
    assert.match(out, /bos-robotstxt <command>/);
  });
  ...
  it('validates and prints the resolved configuration cascade', async () => {
    ...
    assert.match(out, /require_sitemap/);
  });

  it('surfaces a config error as exit code 2', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'bos-robotstxt-cli-'));
    try {
  ...
  it('generates and then audits a robots.txt end to end', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'bos-robotstxt-cli-'));
    try {
      fs.writeFileSync(
        path.join(dir, '.bos-robotstxt.yml'),
        ['robots_txt:', '  groups:', '    - user_agent: "*"', '      disallow: [/admin/]', ''].join(
          '\n',
        ),
        'utf8',
      );
      ...
      const written = fs.readFileSync(path.join(dir, 'site', 'robots.txt'), 'utf8');
      assert.match(written, /Disallow: \/admin\//);
      assert.match(written, /Sitemap: https:\/\/example\.com\/sitemap\.xml/);
  ...
  it('reports a missing robots.txt for audit', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'bos-robotstxt-cli-'));
    try {
      const { code, err } = await runCli(['audit', '--root', dir]);
      assert.strictEqual(code, 2);
      assert.match(err, /robots\.txt not found/);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
