...
const TOOL_NAME = 'BOS Robots TXT Generator';
...
function auditRun(findings, options = {}) {
  ...
  for (const finding of reportable) {
    ...
    rules.push({
      ...
      properties: { tags: ['seo', 'robots-txt', 'rfc9309', 'blackout-secure'] },
    });
  }
  ...
  const results = reportable.map((finding) => ({
    ...
    partialFingerprints: { bosRobotsTxtFindingKey: finding.findingKey },
    properties: {
