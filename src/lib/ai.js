  ...
  if (GITHUB_PROVIDER_ALIASES.includes(name)) {
    ...
    if (token) {
      return {
        ...
        model:
          env.GITHUB_MODELS_MODEL_ROBOTS_TXT || env.GITHUB_MODELS_MODEL || 'openai/gpt-4o-mini',
        token,
      };
    }
    return null;
...
async function summarize(findings, provider, options = {}) {
  ...
  const payload = {
    ...
    messages: [
      { role: 'system', content: 'You are an SEO and crawl-policy triage assistant.' },
      {
        role: 'user',
        content:
          'Summarize these robots.txt audit findings in at most three concise bullets. ' +
...
function localSummary(result) {
  ...
  if (!attention.length) {
    lines.push('- No robots.txt control requires attention in this run.');
    return lines.join('\n');
  }
 ...
 * @param {object} result - An `AuditResult`.
 * @param {object} remediation - Resolved `robots_txt.remediation` config.
