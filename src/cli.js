...
//   generate  Write robots.txt from the resolved configuration.
//   audit     Audit an existing robots.txt against RFC 9309.
...
const USAGE = `bos-robotstxt <command> [options]

Commands:
  version                       Print the package version.
  validate                      Resolve and validate layered configuration.
  generate                      Write robots.txt from the resolved config.
  audit                         Audit an existing robots.txt (RFC 9309).
...
generate options:
  ...
  --site-url <url>              Public base URL (required).

audit options:
  --file <path>                 robots.txt to audit.
  --public-dir <dir>            Directory holding robots.txt.
  --site-url <url>              Public base URL, used for origin checks.
...
function cmdValidate(flags) {
  ...
  const out = [
    ...
    `generate:     comments=${cfg.generate.includeComments} file=${cfg.generate.filename} sitemap=${cfg.generate.includeSitemap}/${cfg.generate.sitemapFilename}`,
    `groups:       ${cfg.groups.length ? cfg.groups.map((g) => g.userAgent).join(', ') : '(none configured)'}`,
    `sitemaps:     ${cfg.sitemaps.length ? cfg.sitemaps.join(', ') : '(none configured)'}`,
    ...
    ...Object.entries(cfg.audit.rules).map(
      ([name, severity]) => `  ${name.padEnd(24)} ${severity}`,
    ),
  ];
...
/**
 * Build robots.txt content from the resolved configuration.
 * @param {object} cfg - Resolved configuration.
 * @param {string} siteUrl - Public base URL.
 * @returns {string} robots.txt content.
 */
function contentFromConfig(cfg, siteUrl) {
  if (!siteUrl) {
    throw new Error('--site-url is required to resolve absolute Sitemap URLs');
  }
  return buildRobotsTxt({
    groups: cfg.groups.map((group) => ({ ...group })),
    sitemaps: resolveSitemaps({
      siteUrl,
      includeSitemap: cfg.generate.includeSitemap,
      sitemapFilename: cfg.generate.sitemapFilename,
      extra: [...cfg.sitemaps],
    }),
...
function cmdGenerate(flags) {
  ...
  const content = contentFromConfig(cfg, flags.siteUrl || '');

  const target = path.join(publicDir, cfg.generate.filename);
...
async function cmdAudit(flags) {
  ...
  if (!fs.existsSync(filePath)) {
    process.stderr.write(`error: robots.txt not found: ${filePath}\n`);
    return 2;
  }
